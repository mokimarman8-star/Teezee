...
public final class Options extends AbstractList<ByteString> implements RandomAccess {
        ...
        private final void buildTrieRecursive(long nodeOffset, Buffer node, int byteStringOffset, List<? extends ByteString> byteStrings, int fromIndex, int toIndex, List<Integer> indexes) {
            ...
            if (byteString.getByte(i6) == byteString2.getByte(i6)) {
                ...
                if (i + 1 == toIndex) {
                    ...
                    buildTrieRecursive(intCount, buffer2, i12, byteStrings, i, toIndex, indexes);
                    node.writeAll(buffer2);
                    return;
                }
            }
            int i13 = 1;
            for (int i14 = i + 1; i14 < toIndex; i14++) {
            ...
            node.writeInt(i13);
            node.writeInt(i2);
            for (int i15 = i; i15 < toIndex; i15++) {
                byte b = byteStrings.get(i15).getByte(i6);
                if (i15 == i || b != byteStrings.get(i15 - 1).getByte(i6)) {
                    node.writeInt(b & 255);
                }
            }
            ...
            while (i < toIndex) {
                ...
                while (true) {
                    if (i17 >= toIndex) {
                        ...
                        if (b2 != byteStrings.get(i17).getByte(i6)) {
                            i3 = i17;
                            break;
                        }
                        i17++;
                ...
                if (i16 == i3 && i6 + 1 == byteStrings.get(i).size()) {
                    node.writeInt(indexes.get(i).intValue());
                    i4 = i3;
                    buffer = buffer3;
                } else {
                    ...
                    i4 = i3;
                    buffer = buffer3;
                    buildTrieRecursive(intCount2, buffer3, i6 + 1, byteStrings, i, i3, indexes);
                }
                buffer3 = buffer;
                i = i4;
                i8 = -1;
    ...
    public /* bridge */ boolean contains(ByteString byteString) {
        return super/*kotlin.collections.AbstractCollection*/.contains((Object) byteString);
    }
