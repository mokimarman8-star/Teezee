package com.bytedance.sdk.component.adexpress;

import android.content.Context;

/* loaded from: /home/user/Teezee-git/app_source/classes3.dex */
public class EjP {
    public static int EjP() {
        com.bytedance.sdk.component.adexpress.Sj.Sj.TKC TKC = com.bytedance.sdk.component.adexpress.Sj.Sj.Sj.Sj().TKC();
        if (TKC == null) {
            return 0;
        }
        return TKC.TzV();
    }

    public static Context Sj() {
        if (com.bytedance.sdk.component.adexpress.Sj.Sj.Sj.Sj().TKC() == null) {
            return null;
        }
        return com.bytedance.sdk.component.adexpress.Sj.Sj.Sj.Sj().TKC().sP();
    }

    public static int TKC() {
        return com.bytedance.sdk.component.adexpress.Sj.Sj.Sj.Sj().TKC().aa();
    }

    public static boolean sP() {
        com.bytedance.sdk.component.adexpress.Sj.Sj.TKC TKC = com.bytedance.sdk.component.adexpress.Sj.Sj.Sj.Sj().TKC();
        return TKC != null && TKC.uA() == 1;
    }
}
